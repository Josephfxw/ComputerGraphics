...
//
void
Quantization::changeThr(int thr)
{
...
//
void
Quantization::quantization(ImagePtr I1, int thr, ImagePtr I2) {
    ...
    // init lookup tables
    int i, lut[MXGRAY];
    double scale = MXGRAY / thr;
    for( i=0; i<MXGRAY; i++) lut[i] = CLIP( scale *int(i/scale) + scale/2.0,0,255);
